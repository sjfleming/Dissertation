...
The stochastic dynamics of the NeutrAvidin-ssDNA complex in MspA can be modeled as a one dimensional drift-diffusion process that obeys the Smoluchowski equation.  The probability that MspA's narrowest constriction is at location $x$ along the ssDNA, having started at position $x_0$, written $p(x,t|x_0)$, obeys
...
\noindent
where $V$ is the bias voltage and $\sigma$ is the effective charge per unit length.
