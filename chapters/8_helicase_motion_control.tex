...
Figure \ref{fig:helicase_force_stepping}a shows a short segment of current versus time data for a single DNA molecule, taken as Dda helicase is forced to step along the DNA by a constant \SI{160}{\mV} applied bias at \SI{37}{\celsius}, unwinding dsDNA duplex ahead of it.  The DNA used in this experiment is sequence \# 0002, along with its complement and tether, listed in Appendix \ref{sequences}.  The same sort of discrete steps in the current are observed as in the case where helicase is driven by ATP (see Figure \ref{fig:helicase_data}, for example).
