...
and the boundary conditions $\tau(0)=\tau(L)=0$.
...
In this case, for a drift-diffusion process, $B(x) = D$, and $A(x) = \frac{F(x)}{\gamma}$, and by the Einstein relation, $\gamma = \frac{k_B T}{D}$, so $A(x) = \frac{D}{k_B T} F(x)$.  Therefore,

\begin{equation}
\psi \, (y) = \exp \left [ \frac{2}{k_B T} \int_0^y  F(x') \,dx' \right ]
\label{eqn:psi2}
\end{equation}
...
\begin{equation}
\tau \, (x_0) = \frac{2}{D} \int_{x_0}^{L} \left ( \frac{1}{\psi(y)} \int_{0}^{y} \psi(z) \,dz \right ) \,dy
\label{eqn:first_passage_sol}
\end{equation}
